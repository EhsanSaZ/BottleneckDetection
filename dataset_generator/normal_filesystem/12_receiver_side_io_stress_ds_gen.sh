...
do
    ...
    #generate a random variable if it is even do the if part 86: "io"
    # number=$RANDOM
    if [ $((number%2)) -eq 0 ]
    then
      clear_all_caches
      echo "Run the java server on receiver side and start metric collector agent";
      ssh root@$receiver_remote_client_ip 'python3 /home/a/AgentMetricCollector/remote_metric_collector.py 86'&
      sleep 5;
      echo "Start collecting metrics on sender side";
      ssh root@$receiver_remote_client_ip 'stress -i 10'&
      sleep 5;
      python3 metric_collector.py 86 &
