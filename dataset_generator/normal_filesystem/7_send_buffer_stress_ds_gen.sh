...
do
    ...
    then
      ...
      ssh root@$receiver_remote_client_ip 'python3 /home/a/AgentMetricCollector/remote_metric_collector.py 59'&
      sleep 5;
    ...
    then
      clear_all_caches
      echo "Run the java server on receiver side and start metric collector agent";
      ssh root@$receiver_remote_client_ip 'python3 /home/a/AgentMetricCollector/remote_metric_collector.py 60'&
      sleep 5;
      echo "Start collecting metrics on sender side";
      cat /proc/sys/net/ipv4/tcp_wmem > tcp_wmem_original_val
      sed -r "/^net.ipv4.tcp_wmem=.*$/d" -i /etc/sysctl.conf
    ...
    then
      clear_all_caches
      echo "Run the java server on receiver side and start metric collector agent";
      ssh root@$receiver_remote_client_ip 'python3 /home/a/AgentMetricCollector/remote_metric_collector.py 61'&
      sleep 5;
      echo "Start collecting metrics on sender side";
      cat /proc/sys/net/ipv4/tcp_wmem > tcp_wmem_original_val
      sed -r "/^net.ipv4.tcp_wmem=.*$/d" -i /etc/sysctl.conf
