...
sender_ip_addr = "134.197.94.169"
sender_port_range = [5000, 5500]

receiver_ip_addr = "127.0.0.1"
receiver_port_range = [50505, 50525]
...
class TransferDiscovery(threading.Thread):
    ...
    def is_transfer_valid(self, local_address_ip, local_address_port, peer_address_ip, peer_address_port):
        if local_address_ip == sender_ip_addr and sender_port_range[0] <= int(local_address_port) <= sender_port_range[1] \
                and peer_address_ip == receiver_ip_addr and receiver_port_range[0] <= int(peer_address_port) <= \
                receiver_port_range[1]:
            return True
        else:
            return False
        ...
        try:
            ...
            while True:
                ...
                for i in range(len(lines)):
                    if sender_ip_addr in lines[i] and receiver_ip_addr in lines[i]:
                        # if True:
...
d = TransferDiscovery(sender_ip_addr, sender_port_range, receiver_ip_addr, receiver_port_range)
d.run()
