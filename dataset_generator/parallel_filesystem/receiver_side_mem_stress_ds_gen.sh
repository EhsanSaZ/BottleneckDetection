...
do
    ...
    #generate a random variable if it is even do the if part 87: "mem"
    # number=$RANDOM
    if [ $((number%2)) -eq 0 ]
    then
      clear_all_caches
      echo "Run the java server on receiver side and start metric collector agent";
      ssh root@$receiver_remote_client_ip 'python3 /users/Ehsan/AgentMetricCollector/remote_parallel_metric_collector.py 87'&
      sleep 5;
      echo "Start collecting metrics on sender side";
      ssh root@$receiver_remote_client_ip "stress-ng --vm-bytes "$(awk '/MemAvailable/{printf "%d\n", $2 * 0.98;}' < /proc/meminfo)"k --vm-keep -m 5"&
      sleep 5;
      python3 parallel_metric_collector.py 87 &
