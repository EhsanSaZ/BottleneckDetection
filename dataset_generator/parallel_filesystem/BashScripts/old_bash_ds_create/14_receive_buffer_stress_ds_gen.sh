...
do
    ...
    #generate a random variable if it is even do the if part 88: tcp receive_buffer_max_value 43690 // 0.5 of default 4096	87380	6291456
    # number=$RANDOM
    if [ $((number%2)) -eq 0 ]
    then
      clear_all_caches
      echo "Run the java server on receiver side and start metric collector agent";
      ssh root@$receiver_remote_client_ip 'python3 /users/Ehsan/AgentMetricCollector/remote_parallel_metric_collector.py 88'&
      sleep 5;
      ...
      ssh root@$receiver_remote_client_ip "echo 'net.ipv4.tcp_rmem= 4096	87380 43690' >> /etc/sysctl.conf && sysctl -p";
      sleep 5;
      python3 parallel_metric_collector.py 88 &
...
#      sysctl -p
    ...
    #generate a random variable if it is even do the if part 89: tcp receive_buffer_max_value 21845 // 0.25 of default
    # number=$RANDOM
    if [ $((number%2)) -eq 0 ]
    then
      clear_all_caches
      echo "Run the java server on receiver side and start metric collector agent";
      ssh root@$receiver_remote_client_ip 'python3 /users/Ehsan/AgentMetricCollector/remote_parallel_metric_collector.py 89'&
      sleep 5;
      echo "Start collecting metrics on sender side";
      ssh root@$receiver_remote_client_ip 'cat /proc/sys/net/ipv4/tcp_rmem > ./receiver/tcp_rmem_original_val && sed -r "/^net.ipv4.tcp_rmem=.*$/d" -i /etc/sysctl.conf';
      ssh root@$receiver_remote_client_ip "echo 'net.ipv4.tcp_rmem= 4096	87380 21845' >> /etc/sysctl.conf && sysctl -p";
      sleep 5;
      python3 parallel_metric_collector.py 89 &
      sleep $main_sleep_time;
      kill_all_java_python3_processes
      ssh root@$receiver_remote_client_ip 'sed -r "/^net.ipv4.tcp_rmem=.*$/d" -i /etc/sysctl.conf && echo "net.ipv4.tcp_rmem= " $(cat ./receiver/tcp_rmem_original_val) >> /etc/sysctl.conf && rm ./receiver/tcp_rmem_original_val && sysctl -p';
      sleep 5;
    fi
    #generate a random variable if it is even do the if part 90: tcp receive_buffer_max_value 10922 // 0.25 of default
    # number=$RANDOM
    if [ $((number%2)) -eq 0 ]
    then
      clear_all_caches
      echo "Run the java server on receiver side and start metric collector agent";
      ssh root@$receiver_remote_client_ip 'python3 /users/Ehsan/AgentMetricCollector/remote_parallel_metric_collector.py 90'&
      sleep 5;
      echo "Start collecting metrics on sender side";
      ssh root@$receiver_remote_client_ip 'cat /proc/sys/net/ipv4/tcp_rmem > ./receiver/tcp_rmem_original_val && sed -r "/^net.ipv4.tcp_rmem=.*$/d" -i /etc/sysctl.conf';
      ssh root@$receiver_remote_client_ip "echo 'net.ipv4.tcp_rmem= 4096	87380 10922' >> /etc/sysctl.conf && sysctl -p";
      sleep 5;
      python3 parallel_metric_collector.py 90 &
      sleep $main_sleep_time;
      kill_all_java_python3_processes
      ssh root@$receiver_remote_client_ip 'sed -r "/^net.ipv4.tcp_rmem=.*$/d" -i /etc/sysctl.conf && echo "net.ipv4.tcp_rmem= " $(cat ./receiver/tcp_rmem_original_val) >> /etc/sysctl.conf && rm ./receiver/tcp_rmem_original_val && sysctl -p';
      sleep 5;
