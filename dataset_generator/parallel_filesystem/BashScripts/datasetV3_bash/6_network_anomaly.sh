...
#levels=(["17"]=0.1 ["18"]=0.5 ["19"]=1 ["20"]=2)
levels=(["17"]=2  ["18"]=3 ["19"]=4 ["20"]=5)
...
do
    ...
    for i in "${!levels[@]}"; do
      ...
      then
        if [ ${i} == "17" ]; then
          sudo tc qdisc add dev $ethernet_interface_name root netem delay 12ms ${levels[$i]}ms distribution normal;
        fi
        if [ ${i} == "18" ]; then
          sudo tc qdisc add dev $ethernet_interface_name root netem delay 13ms ${levels[$i]}ms distribution normal;
        fi
        if [ ${i} == "19" ]; then
          sudo tc qdisc add dev $ethernet_interface_name root netem delay 14ms ${levels[$i]}ms distribution normal;
        fi
        if [ ${i} == "20" ]; then
          sudo tc qdisc add dev $ethernet_interface_name root netem delay 15ms ${levels[$i]}ms distribution normal;
        fi
      else
        if [ ${i} == "17" ]; then
          sudo tc qdisc add dev $ethernet_interface_name root netem delay 12ms ${levels[$i]}ms distribution normal;
