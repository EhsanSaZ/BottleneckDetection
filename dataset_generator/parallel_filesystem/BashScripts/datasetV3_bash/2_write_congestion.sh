...
# write_congestion_by_clients_on_receiver_ost
#congestions=([1]="write_congestion_by_clients_on_receiver_ost_1" [2]="write_congestion_by_clients_on_receiver_ost_2"
#[3]="write_congestion_by_clients_on_receiver_ost_3" )

congestions=([1]="4" [2]="5" [3]="6")
while true
do
    printf "\n"
...
#        sudo tc qdisc add dev $ethernet_interface_name root netem rate ${network_bw};
      ...
      if [ ${i} == "1" ]; then
         ...
         ssh root@$remote_client1_ip "python3 /users/Ehsan/AgentMetricCollector/DiskRWStress/write_test.py /lustre/OST_1/diskWriteStress/cli1 1"&
      fi
      if [ ${i} == "2" ]; then
