...
# Lustre_Limk_congestion_on_sender_side
#congestions=([1]="Lustre_Limk_congestion_on_sender_side_1" [2]="Lustre_Limk_congestion_on_sender_side_2"
#[3]="Lustre_Limk_congestion_on_sender_side_3")

#CONGESTION LEVELS FOR SSD DISK
#congestions=([2]="7" [3]="8" [4]="9")
#CONGESTION LEVELS FOR HDD DISK
congestions=([4]="7" [6]="8" [8]="9")
...
#        sudo tc qdisc add dev $ethernet_interface_name root netem rate ${network_bw};
        sudo tc qdisc add dev $ethernet_interface_name root netemdelay 30ms;
      fi
      sleep 8;

      if [ ${i} == "4" ]; then
         echo "Congestion lnk with -P level 1"
         iperf3 -s >> /dev/null & sleep 5; ssh root@$server3 "iperf3 -c 10.10.1.5 -t 10000 -P ${i} >> /dev/null"&
      fi
      if [ ${i} == "6" ]; then
