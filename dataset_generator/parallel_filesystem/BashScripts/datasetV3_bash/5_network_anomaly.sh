...
#levels=([1]=0.05 [2]=0.1 [3]=0.5 [4]=1)
# wisconsin
levels=(["13"]=0.2 ["14"]=0.3 ["15"]=0.5 ["16"]=1)
# utah
#levels=(["13"]=0.6 ["14"]=0.8 ["15"]=1 ["16"]=1.2)
...
do
    ...
    for i in "${!levels[@]}"; do
      ...
      if $limited_bw
      then
        sudo tc qdisc add dev $ethernet_interface_name root netem loss ${levels[$i]}% rate ${network_bw};
      else
        sudo tc qdisc add dev $ethernet_interface_name root netem loss ${levels[$i]}%;
      fi
      sleep 8;
