...
from model.log_metrics_pb2 import PublisherPayload
...
def worker_routine(worker_url, context=None):
    ...
    db_client = MongoClient('mongodb://{}:{}/{}'.format("c220g1-030814.wisc.cloudlab.us", "27017", "testdb"),username="mongoadmin", password="mongoadmin", )
    ...
    with ThreadPoolExecutor(max_workers=worker) as executor:
        while True:
            ...
            if socks.get(sub_socket) == zmq.POLLIN:
                ...
                if payload.request_type == "unsubscribe_publisher_info":
                    sender_ip = payload.ip
                    sender_port = payload.port
                    sub_socket.disconnect("tcp://{}:{}".format(sender_ip, sender_port))
                else:
                    # process the event
                    executor.submit(process_event_v2, payload, db_client, db_name)
